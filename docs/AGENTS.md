...
* **BAAQMD Bay Area Refinery Flare Incident Connector (`src/baaqmd_flares.py` - Issue #385):**
  - **Rule 12-11/12 Investigation Ingestion:** Ingests official flare causal investigation reports from the Bay Area Air Quality Management District across Northern California refineries (Chevron Richmond, Valero Benicia, Phillips 66 Rodeo, PBF Martinez).
  - **Structured Root Cause Categorization:** Categorizes unit upsets (`unplanned_power_outage`, `compressor_trip`, `unit_turnaround`, `flaring_safety_relief`), flare volume (scf), and $\text{SO}_2$ emissions into an immutable retrospective benchmark dataset (`data/benchmarks/baaqmd_refinery_events.csv`) for detector validation and scenario parameter calibration.
* **NASA POWER Meteorological & Agroclimatological Engine (`src/nasa_power.py` - Issues #420 & #370):**
  - **PADD 1 Distillate HDD/CDD Normalization:** Ingests NASA POWER satellite-calibrated spatial daily meteorology across critical East Coast fuel hubs (New York Harbor, Delaware City Refinery, Boston Harbor) to calculate Heating Degree Days ($HDD = \max(0, 65 - T_{\text{mean}})$) and Cooling Degree Days ($CDD = \max(0, T_{\text{mean}} - 65)$) for baseline prompt Heating Oil / ULSD distillate crack spread modeling.
  - **PADD 2 Midwest Biofuel & Ethanol Agroclimatology:** Ingests satellite surface solar irradiance ($\text{ALLSKY\_SFC\_SW\_DWN}$ in $\text{MJ/m}^2/\text{day}$), precipitation, and Growing Degree Days ($GDD = \max(0, \frac{\min(86, T_{\text{max}}) + \max(50, T_{\text{min}})}{2} - 50)$) across major Corn Belt agricultural hubs (Des Moines IA, Peoria IL, Omaha NE) to model agricultural feedstock yields and seasonal ethanol refining economics ($0.90 \times \text{RBOB} + 0.10 \times \text{Ethanol} + \text{RINs}$).
