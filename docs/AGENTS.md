...
* **Standardized 5-Tier Nested Model Evaluation Hierarchy & Statistical Promotion Protocol (`src/model_evaluation.py` & `scripts/evaluate_model_hierarchy.py` - Issue #362):**
...
* **Unified Intraday Event Monitor & Webhook Gateway (`src/intraday_event_monitor.py`):**
  ...
  - **Cloudflare Edge Workers, Queues & Staging Isolation Architecture (`workers/intraday_monitor_worker.ts`, `workers/cache_worker.ts`, `wrangler.toml`, & `workers/wrangler.cache.toml` - Issues #194, #438):**
    - **`midgley-intraday-monitor` (`workers/intraday_monitor_worker.ts` - Issue #438):** 15-minute cron trigger worker scanning energy RSS feeds, evaluating contextual regex anomaly triggers, deduplicating via global Cloudflare D1 database (`seen_rss_headlines` in `midgley-cache-d1`) with 24-hour TTL, and firing GitHub Repository Dispatch events or enqueuing to edge queues. Enforces authentication on `POST /flag`, `POST /run`, and `POST /trigger` endpoints and HTML-escapes all reflected query inputs in `GET /flag`.
    ...
    - **`midgley-cache-worker` (`workers/cache_worker.ts` - Issue #438):** Tier 2 Edge Cache Gateway exposing `/api/v1/cache/:key` GET/POST and `/status` REST endpoints over Cloudflare D1. Enforces fail-closed token validation via `CLOUDFLARE_AUTH_TOKEN` (rejects non-health requests with `401 Unauthorized` if token is missing or invalid).
...
* **Self-Hosted Process Concurrency & Timer Serialization (`scripts/run_local_*.sh` & `systemd/`, Issue #425):**
  - **Advisory Lock Barrier (`flock`):** Background scheduled runners (`midgley-daily-forecast.service`, `midgley-intraday-polling.service`, `midgley-weekly-review.service`) serialize access to `data/` using an advisory lock (`exec 9>"${XDG_RUNTIME_DIR:-/tmp}/midgley-data.lock"` and `flock -w 900 9`). Competing pipeline units queue up to 15 minutes and exit with code 75 (`EX_TEMPFAIL`) on timeout to prevent silent lost updates.
  - **Timer Jitter & Offset Schedules:** Systemd timers carry `RandomizedDelaySec=120` to prevent simultaneous execution spikes, and the intraday poller operates on an offset schedule (`OnCalendar=*:7/15`), eliminating deterministic top-of-hour collisions with daily (07:00 UTC) and weekly (13:00 UTC) runs.
  - **Resilient API Reader Path:** `src/api_server.py` guards CSV reading against `(FileNotFoundError, pd.errors.EmptyDataError, pd.errors.ParserError, OSError)` to guarantee unbroken 200 API responses during initial boots or live atomic swaps.
