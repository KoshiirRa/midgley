...
- **Card Rendering**: Use `render_regional_driver_cards_html(region_id)` from [`src/regional_metadata.py`](src/regional_metadata.py) inside [`src/dashboard_generator.py`](src/dashboard_generator.py) to dynamically construct responsive Tailwind visual cards covering all 4 core dimensions. Never hardcode prose driver descriptions inside Python HTML template strings.
