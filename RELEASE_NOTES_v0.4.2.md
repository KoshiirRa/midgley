...
### 17. SEC EDGAR 8-K Refinery Operator Monitor (Issue #129)
- **Zero-Cost EDGAR Feed (`src/edgar_8k_monitor.py` & `workers/intraday_monitor_worker.ts`):** Implemented edge-first EDGAR 8-K filing monitor using the SEC EDGAR ATOM RSS API (`cgi-bin/browse-edgar?output=atom`). In production, the Cloudflare Worker (`workers/intraday_monitor_worker.ts`) handles polling and deduplication at the edge via a new `pollEdgar8KFeeds()` function wired into the existing 15-minute cron trigger. The Python module (`src/edgar_8k_monitor.py`, `EDGAR8KMonitor` class) serves as the origin queue-consumer handler and local dev/fallback harness. Zero new Python dependencies — stdlib only (`urllib`, `xml.etree.ElementTree`, `html.parser`).
