...
- **Standalone Web Portal ([`docs/citations.html`](docs/citations.html)):** Built a responsive Tailwind + KaTeX academic literature portal documenting all 12 peer-reviewed research papers and foundation algorithms powering the Midgley forecasting architecture.
...
- **Streamlined Math Guide Integration & Hero Card Integration ([`docs/math.html`](docs/math.html)):** Removed redundant legacy Section 11 from the math page and integrated dual CTA buttons (*Open Citations Portal* & *RESEARCH_CITATIONS.md*) directly into the top Hero banner. Cleanly renumbered subsequent modeling sections (ULSD -> Section 11, DV-GPB -> Section 12, USGS Attenuation -> Section 13, Qlib Alpha Mining -> Section 14).
- **Global Header Navigation Parity & Dynamic Generator Integration:** Updated `get_nav_header()` in [`src/dashboard_generator.py`](src/dashboard_generator.py) to dynamically render the `Citations` navigation link and handle `active_page == 'citations'` state across all 13 generated pages and metro subdirectories (`index.html`, `national.html`, `math.html`, `telemetry.html`, `diesel.html`, `savings.html`, `tulsa.html`, `newark.html`, `cincinnati.html`, `greenville.html`, `charlotte.html`, `bayarea.html`, `port_st_lucie.html`).
- **End-to-End Master Prediction Synthesis Section (Section 15 on [`docs/math.html`](docs/math.html)):** Implemented a culminating master econometric composition section featuring **Equation 15.1** (\(\hat{P}_{r, t+5}\)) synthesizing Quantitative Core futures, Decayed Shock Memory (\(\mathbf{M}_t\)), Qlib Symbolic Alphas, Regional Basis (\(\Delta_{\text{metro}, r}\)), Statutory State/CARB Taxes (\(T_{\text{tax}, r}\)), Physical Friction (\(\delta_{\text{physical}, r}\)), Volatility Gating (\(\lambda_{\text{vol}}\)), and Empirical Residual Confidence Bands (\(\text{CI}_{95\%}\)) alongside a structured factor-by-factor architectural attribution matrix.
- **Unified Weather Vulnerability Matrix (Section 03 on [`docs/math.html`](docs/math.html), Issue #222):** Synchronized **Equation 3.1** and the Tier 2 localized weather risk taxonomy to comprehensively represent all 7 active regional metro hubs (Tulsa `OKZ060/OKZ066`, Newark `DEZ001`, Cincinnati `OHZ077/KYZ091`, Greenville `NCZ081`, Charlotte `NCZ071`, Oakland/Bay Area `CAZ508/CAZ511`, and Port St. Lucie `FLZ147`) with localized convective, hydrologic, Nor'easter, bomb cyclone, and tropical storm hazard parameters.
...
- **Comprehensive Waterway & Terminal Modeling ([`docs/math.html`](docs/math.html)):** Expanded Section 04 from global maritime chokepoints to **"Global & Regional Maritime Chokepoints, Inland River Barging & Waterborne Terminals"**, bringing theoretical parity with live data ingestion pipelines across all 7 regional metro calibration hubs.
...
- **Typing Import Resolution ([`src/data_ingestion.py`](src/data_ingestion.py)):** Added `Optional, Union, Callable` to the top-level typing imports in `src/data_ingestion.py`, resolving runtime `NameError: name 'Optional' is not defined` on Python 3.13 when evaluating `OpenSourceAIRadarConnector` type annotations.
- **Explicit NetworkX & Core Tooling Dependencies ([`requirements.txt`](requirements.txt) & [`pyproject.toml`](pyproject.toml)):** Explicitly added `networkx>=3.0`, `scipy>=1.12.0`, `python-dotenv>=1.0.0`, `pydantic>=2.0.0`, and `beautifulsoup4>=4.12.0` to project dependencies, ensuring headless and fresh runner environments have graph topology and mathematical optimization engines pre-installed.
- **Workflow Pipeline Exit Code Hardening ([`.github/workflows/gas_price_forecast.yml`](.github/workflows/gas_price_forecast.yml)):** Added `set -eo pipefail` to the daily forecasting execution step to guarantee non-zero python exit codes are never masked by `tee`, preventing false-positive CI green states and ensuring automated alerts trigger on pipeline failures.
