...
def run_charlotte_pipeline(live_pump_price: float = None, use_llm_api: bool = False, model_type: str = "ridge"):
    ...
    print("\n[Step 3/6] Engineering Paw Creek Rack Crack Spread & Fusing Decayed Event Memory (Multi-Horizon 1D-5D)...")
    ...
    print("\n" + "=" * 65)
    print("     CHARLOTTE REGIONAL MODEL EVALUATION & METRICS SUMMARY (5-DAY)")
    print("=" * 65)
    ...
    for h in [1, 2, 3, 4, 5]:
        ...
        hist_charlotte_pred = hist_charlotte_base * (1.0 + pred_ret_hybrid)
        hist_charlotte_quant = hist_charlotte_base * (1.0 + pred_ret_quant)
        ...
        log_predictions(today_df, region="Charlotte_NC", model_version=charlotte_version, run_type="LIVE_PROSPECTIVE", forecast_horizon_days=h, is_retroactive_backtest=False)
