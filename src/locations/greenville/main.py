"""
Greenville Regional Master Execution Script (src/locations/greenville/main.py)
...
def run_greenville_pipeline(live_pump_price: float = None, use_llm_api: bool = False, model_type: str = "ridge"):
    ...
    print("\n[Step 3/6] Engineering Selma Rack Crack Spread & Fusing Decayed Event Memory (Multi-Horizon 1D-5D)...")
    ...
    print("\n" + "=" * 65)
    print("     GREENVILLE REGIONAL MODEL EVALUATION & METRICS SUMMARY (5-DAY)")
    print("=" * 65)
    ...
    for h in [1, 2, 3, 4, 5]:
        ...
        hist_greenville_base = h_splits['test_df']['greenville_retail_gasoline'] if 'greenville_retail_gasoline' in h_splits['test_df'].columns else (h_splits['test_df']['gasoline_rbob'] + dynamic_margin)
        ...
        hist_greenville_pred = hist_greenville_base * (1.0 + pred_ret_hybrid)
        hist_greenville_quant = hist_greenville_base * (1.0 + pred_ret_quant)
        ...
        backfill_new_region_history(
            ...
            quant_baseline_prices=hist_greenville_quant
        )
        ...
        greenville_h_forecast = live_pump_price * (1.0 + baseline_return_h)
        greenville_h_quant = live_pump_price * (1.0 + quant_return_h)

        today_df = pd.DataFrame([{
            'date': last_date,
            'current_price': live_pump_price,
            'predicted_5d_price': greenville_h_forecast,
            'quant_baseline_5d_price': greenville_h_quant,
            'forecast_horizon_days': h,
            'is_retroactive_backtest': False
        }])
        log_predictions(today_df, region="Greenville_NC", model_version=greenville_version, run_type="LIVE_PROSPECTIVE", forecast_horizon_days=h, is_retroactive_backtest=False)
