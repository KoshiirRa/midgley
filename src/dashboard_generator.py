"""
Public Dashboard Generator (src/dashboard_generator.py)
Generates docs/index.html for GitHub Pages public hosting.
Provides both Non-Technical Executive Views, Technical MLOps Analytics, and
a detailed Predictive Logic & Methodology documentation section at the bottom.
"""
...
def generate_public_dashboard():
    ...
    if os.path.exists(HISTORY_CSV_PATH):
        try:
            ...
            history_json = df.tail(100).to_dict(orient="records")
        except Exception as e:
            logger.warning(f"Could not load prediction history for dashboard: {e}")
    ...
    <header class="border-b border-slate-800 bg-slate-900/80 backdrop-blur sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div class="flex items-center gap-3 text-sm">
                <span class="text-xs text-slate-400"><i class="fa-regular fa-clock mr-1"></i> Updated: {now_str}</span>
        ...
        <section class="p-8 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-6">
            <div class="flex items-center gap-3 border-b border-slate-800 pb-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-400">How quantitative time-series data, LLM news extraction, and NOAA weather intelligence combine to form 5-day forecasts</p>
                </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                
                <!-- Pillar 1: Quantitative Commodity Foundations -->
                ...
                <div class="p-5 rounded-xl bg-slate-950 border border-slate-800/80 space-y-3">
                    ...
                    <p class="text-xs text-slate-300 leading-relaxed">
                        Raw, unstructured news headlines and OPEC press releases are analyzed by <strong>Google Gemini 2.5 Flash</strong>. The LLM translates qualitatively complex geopolitical events into bounded numerical impact vectors: <code class="text-emerald-300">geopolitical_risk</code> [-1.0, +1.0], <code class="text-emerald-300">supply_disruption</code> [0.0, +1.0], and <code class="text-emerald-300">overall_price_pressure</code> [-1.0, +1.0].
                    </p>
                </div>

                <!-- Pillar 3: NOAA Weather Model Integration -->
                <div class="p-5 rounded-xl bg-slate-950 border border-slate-800/80 space-y-3">
                    ...
                    <p class="text-xs text-slate-300 leading-relaxed">
                        Weather shocks are fetched from the NOAA National Weather Service API (<code class="text-amber-300">api.weather.gov</code>). We track two geographic tiers: <strong>National Production Basins</strong> (Gulf Coast Hurricanes & Permian Freezes) and <strong>Localized Tulsa Zones</strong> (Tulsa County <code class="text-amber-300">OKZ060</code> Tornado Warnings & Cushing/Payne County <code class="text-amber-300">OKZ066</code> Hard Freeze Warnings).
                    </p>
                </div>

                <!-- Pillar 4: Exponential Memory Decay -->
                <div class="p-5 rounded-xl bg-slate-950 border border-slate-800/80 space-y-3">
                    ...
                    <p class="text-xs text-slate-300 leading-relaxed">
                        To prevent news shocks from acting as unrealistic single-day spikes, event impact scores pass through an exponential decay filter: 
                        Memory_t = Memory_{{t-1}} * exp(-lambda) + Shock_t.
                        This models how market participants digest news over a 2 to 3 week window.
                    </p>
                </div>

            </div>

            <!-- Pillar 5 & 6: Live Pump Calibration & Machine Learning Model -->
            <div class="p-5 rounded-xl bg-slate-950 border border-slate-800 space-y-3">
                ...
                <p class="text-xs text-slate-300 leading-relaxed">
                    Rather than predicting non-stationary raw price levels, our model trains a standardized <strong>Ridge Regression ($\alpha=10.0$)</strong> pipeline to predict <strong>5-day percentage price returns</strong> ($\Delta \%$). The predicted percentage shift is applied directly to today's live pump price (<strong>$3.89/gal</strong> in Tulsa), ensuring that 5-day forecasts and scenario shocks adjust dynamically relative to real pump prices at local stations today.
                </p>
            </div>
        </section>
