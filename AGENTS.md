...
* **Zero-Cost Open-Access Energy Data Suite & Universal 50-State Connector (`src/data_ingestion.py`, `src/state_open_data.py`, & `src/noaa_weather.py`) (Issue #141):**
  ...
  - **Multi-Feed Air Quality (AQI) Ingestion & Refinery Outage Early Detection (`src/aqi_feed.py`) (Issue #54):** `AQIFeedConnector` ingests real-time and historical air quality metrics ($\text{PM}_{2.5}$, $\text{PM}_{10}$, $\text{SO}_2$, $\text{NO}_2$, $\text{O}_3$) from government (EPA AirNow) and independent crowdsourced/open sensor networks (PurpleAir, OpenAQ, WAQI) within 15 km fence-line polygons downwind of critical refining hubs (SF Bay Area Chevron Richmond/PBF Martinez/Valero Benicia, Tulsa HF Sinclair, Delaware Valley PBF Delaware City/Bayway, and Ohio Valley Marathon Catlettsburg). Computes standardized rolling 30-day $Z$-scores ($Z_{\text{PM2.5}} \ge 3.5$ and $Z_{\text{SO2}} \ge 2.5$) to detect catastrophic catalytic cracker trips and emergency flaring with 12–24 hour lead time over commercial news, while discriminating against non-refinery smoke (wildfires) via $\text{SO}_2$ co-detection, feeding `aqi_bay_area_outage_risk_index`, `aqi_tulsa_outage_risk_index`, `aqi_delaware_outage_risk_index`, `aqi_catlettsburg_outage_risk_index`, and `composite_aqi_shock_index` into `src/feature_engineering.py` and regional metro models.
...
* **Continuous Feedback Loop & Self-Review Mechanism:**
  ...
  - **Automated CORE Open-Access Research Paper Monitor (`src/core_monitor.py`, Issue #53):** Queries `api.core.ac.uk/v3/search/works` for recent open-access research papers in energy commodity forecasting, refining rack margins, and machine learning time-series literature matching domain queries within the 7-day review window, formatting abstracts, DOIs, and PDF download links into weekly review reports.
