...
* **Functions:**
  ...
  - `backfill_actual_prices_and_evaluate(target_region=None)`: Queries real ground-truth retail prices via `EIARetailFeed` and national futures from yfinance (with local disk caching in `data/rbob_actuals_cache.json`), completely eliminating synthetic offset ladders (`RB=F + 0.55`) and identity fallbacks (`base_price - raw_actual`) (Issues #391, #392). Unmapped or missing observations remain unobserved (`np.nan`) rather than forcing artificial directional hits. Evaluates 95% CI coverage hits, backfills actual prices in `prediction_history.csv`, scopes episodic memory shock retention to `target_region` (Issue #326), and triggers background cloud DB sync.
