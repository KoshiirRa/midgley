...
* **TCEQ Air Emissions Events & Gulf Coast Disruption Connector (`src/tceq_emissions.py` - Issue #382):**
  ...
  - **Empirical Outage Benchmark Matrix:** Catalogs verified historical flaring events with $\text{SO}_2$ and $\text{VOC}$ emitted volumes, affected process units, and disruption severities in `data/benchmarks/tceq_emissions_events.csv` and `data/tceq_emissions_vintages.json`.
