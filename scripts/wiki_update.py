...
* **GET /api/v1/forecast/scoreboard**: Continuous MLOps rolling model accuracy scoreboard returning 30/60/90-day MAE, RMSE, MAPE, Directional Hit Rate %, Naive Persistence Baseline MAE, Model MAE Uplift %, Model vs. Persistence Win Rate %, and LLM vs. Quant Win Rate % vs. ground-truth market actuals (supports optional `?include_retroactive=true` for retroactive backtest auditing, Issue #389).
